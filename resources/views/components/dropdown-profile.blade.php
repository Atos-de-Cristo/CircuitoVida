...
<div class="relative inline-flex" x-data="{ open: false }">
    ...
    >
        ...
        <div class="flex items-center truncate">
            <span class="truncate ml-2 text-sm font-medium group-hover:text-slate-800 dark:group-hover:text-white hidden md:block">{{ Auth::user()->name }}</span>
            <svg class="w-3 h-3 shrink-0 ml-1 fill-current text-slate-400 hidden md:block" viewBox="0 0 12 12">
    ...
    <div
        ...
        @keydown.escape.window=hidden md:block"open = false"
        x-show="open"
    ...
    >
        <div class="pt-0.5 pb-2 px-3 mb-1 border-b border-slate-200">
            ...
            <div class="text-xs text-slate-500 italic">{{ Auth::user()->permissions()->implode('permission', ',') }}</div>
        </div>
        <ul>
            <li>
