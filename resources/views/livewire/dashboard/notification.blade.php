<div>
    <div class="relative inline-flex" x-data="{ open: false }">
        ...
        <div
            class="origin-top-right z-10 absolute top-full -mr-48 sm:mr-0 min-w-80 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 py-1.5 rounded shadow-lg overflow-hidden mt-1 right-0"
            ...
            x-cloak
        >
            <div class="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase pt-1.5 pb-2 px-4">Notificações</div>
            <ul>
                @forelse ($this->listMessage as $message)
                <li class="border-b border-slate-200 dark:border-slate-700 last:border-0">
                    <a class="block py-2 px-4 hover:bg-slate-50 dark:hover:bg-slate-700/20"  wire:click="read({{ $message->id }})">
                        <span class="block text-sm mb-2">
                            <small>De: {{$message->userSend->name}}</small>
                            <br>
                            <p>{{$message->message}}</p>
                        </span>
                        <span class="block text-xs font-medium text-slate-400 dark:text-slate-500">{{\Carbon\Carbon::parse($message->date_send)->format('d/m/Y H:i:s')}}</span>
                    </a>
                </li>
                @empty
                <li class="border-b border-slate-200 dark:border-slate-700 last:border-0">
